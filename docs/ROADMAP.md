...
| **I.5** | **Prospeo** — second live paid email resolver (`enrich-person`) behind the shared `DataProvider` interface + BYO connect card in Settings. Contact Intelligence waterfall now falls through Hunter → Prospeo (cheapest-first, stops at first hit) before giving up. The contact drawer renders a per-contact "Enrich" button for **every** connected email partner (Hunter, Prospeo), not just Hunter. | shipped |
...
| **I.7** | **Async provider interface + Dropcontact.** `DataProvider` gains `submitEnrich`/`pollEnrich`/`enrichIsAsync`; `runProviderEnrich` submits+polls async providers to a time budget and passes through sync ones. Dropcontact (async batch enrich) wired for per-contact drawer enrich; kept out of the bulk discovery loop for latency. | shipped |
| **I.8** | **Apollo** — live `people/match` enrichment (was a stub). Drops Apollo's `email_not_unlocked` placeholder instead of saving a junk address. Added to the discovery waterfall chain (Hunter → Apollo → Prospeo) and the drawer enrich buttons. Live email partners now: Hunter, Apollo, Prospeo, Dropcontact. | shipped |
| **I.9** | **Free-path discovery fixes** (post-deploy troubleshooting: "new york times" → 0 results). Real domain resolution via known-outlet seed map + Google News `<source url>` (replaces the broken `.com` slug guess); feed probing (known publisher feeds + common paths) since big outlets advertise nothing on their homepage; journalist mining from outlet article pages' JSON-LD/meta bylines; multi-author byline splitting; 5-min negative crawl cache (was 1h); honest results-panel labeling (free vs paid); Build-target-list copy clarifies it ranks the existing directory. | shipped |
